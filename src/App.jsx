...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .modal { background: white; border-radius: 24px 24px 0 0; width: 100%; max-width: 480px; margin: 0 auto; padding: 1.5rem; max-height: 80vh; overflow-y: auto; animation: slideUp 0.3s cubic-bezier(0.32,0.72,0,1); }
  ...
  .pay-btn { width: 100%; padding: 1rem; background: var(--gold); color: var(--dark); border: none; border-radius: 12px; font-size: 1rem; font-weight: 700; cursor: pointer; margin-top: 1rem; font-family: 'DM Sans', sans-serif; transition: background 0.2s; }
  ...
  .qr-code-box { display: inline-block; padding: 1rem; background: white; border-radius: 12px; border: 2px solid var(--border); margin: 1rem auto; }
...
// ─── QR CODE (SVG maison, pas de lib externe) ─────────────────────────────────
function SimpleQR({ url }) {
  return (
    <div className="qr-code-box">
      <img
        src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(url)}&bgcolor=ffffff&color=1A1208`}
        alt="QR Code"
        width={180}
        height={180}
...
function TableSelector({ onSelect }) {
  return (
    <div className="table-select-wrap">
      ...
      <p style={{ color: "var(--warm-gray)", fontSize: "0.85rem", marginTop: "0.5rem" }}>
...
function ClientView() {
  ...
  const handlePay = async () => {
    setPaying(true);
    const items = menu.filter(i => cart[i.id] > 0).map(i => ({ name: i.name, qty: cart[i.id], price: i.price, emoji: i.emoji }));
    const { error } = await supabase.from('orders').insert({ table_num: tableNum, items, total: totalPrice, paid: true, status: 'en attente' });
    if (!error) { setSuccess(true); setCart({}); }
    setPaying(false);
  };
  ...
  if (success) return (
    <div className="client-wrap">
      <div className="success-box">
        ...
        <p className="success-sub">Votre commande est en cours de préparation.<br />Un serveur vous apportera vos plats sous peu.</p>
        <button className="pay-btn" style={{ marginTop: "1.5rem", maxWidth: 240 }} onClick={() => { setSuccess(false); setTableNum(null); }}>
          Nouvelle commande
        </button>
      </div>
    </div>
      ...
      {showCart && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowCart(false)}>
          <div className="modal">
            ...
            {menu.filter(i => cart[i.id] > 0).map(item => (
              <div key={item.id} className="cart-line">
                <span>{item.emoji} {item.name} × {cart[item.id]}</span>
                <span style={{ fontWeight: 600 }}>{(item.price * cart[item.id]).toFixed(2)} €</span>
              </div>
            ))}
            ...
            <button className="pay-btn" onClick={handlePay} disabled={paying}>
              {paying ? '⏳ Envoi en cours...' : `✅ Commander — ${totalPrice.toFixed(2)} €`}
            </button>
          </div>
        </div>
      )}
...
function KitchenView() {
  ...
  const fetchOrders = useCallback(async () => {
    const { data } = await supabase.from('orders').select('*').neq('status', 'servi').order('created_at', { ascending: true });
    if (data) setOrders(data);
  }, []);

  useEffect(() => {
    fetchOrders();
    const channel = supabase.channel('kitchen').on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, fetchOrders).subscribe();
  ...
  return (
    <div className="kitchen-wrap">
      ...
      <div className="view-sub">{orders.length} commande{orders.length !== 1 ? 's' : ''} en cours — mis à jour en temps réel</div>
      {orders.length === 0 ? (
...
function AdminView() {
  ...
  const pending = orders.filter(o => o.status !== 'servi').length;
